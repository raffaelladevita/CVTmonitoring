 ...
 * @author devita
 */
public class PullsModule extends Module {
    ...
    public DataGroup svtLayerGroup(int layer) {
        ...
        for (int is = 0; is < Constants.SVTSECTORS[layer-1]; is++) {
            ...
            H1F hi_res = histo1D("hi_res_" + name, name + " Centroid Pull", "Counts", 100, -SMAX, SMAX, 3);
    ...
    public DataGroup svtGraphGroup() {
        ...
        for (int il = 0; il < Constants.SVTLAYERS; il++) {
            ...
            graph.setTitleY("Centroid Pull (mm)");
    ...
    public DataGroup bmtLayerGroup(String detector) {
        ...
        for (int ir = 0; ir < Constants.BMTREGIONS; ir++) {
            for (int is = 0; is < Constants.BMTSECTORS; is++) {
                ...
                H1F hi_res = histo1D("hi_res_" + name, name + " Centroid Pull", "Counts", 100, -BMAX, BMAX, 3);
    ...
    public DataGroup sumGroup(double max) {
        ...
        H2F hi_res_p     = histo2D("hi_res_p",     "p (GeV)",      "Centroid Pull", 100,  0.0, 2.0, 100, -max, max);
        H2F hi_res_theta = histo2D("hi_res_theta", "#theta (deg)", "Centroid Pull", 100,   20, 140, 100, -max, max);
        H2F hi_res_phi   = histo2D("hi_res_phi",   "#phi (deg)",   "Centroid Pull", 100, -180, 180, 100, -max, max);
    ...
    public void fillHistos(Event event) {
        for(Cluster cluster : event.getClusters()) {
            if(cluster.getTrackId()>0) {
                ...
                this.getHistos().get(detector + "sum").getH1F("hi_res").fill(pull);
                this.getHistos().get(detector + "sum").getH2F("hi_res_p").fill(track.p(), pull);
                this.getHistos().get(detector + "sum").getH2F("hi_res_theta").fill(Math.toDegrees(track.theta()), pull);
                this.getHistos().get(detector + "sum").getH2F("hi_res_phi").fill(Math.toDegrees(track.phi()), pull);
            }
        }
    }
